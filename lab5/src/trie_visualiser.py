...
class TrieVisualizer:
    ...
    def _add_node(self, node, parent_name=None):
        ...
        if node.terminate:
            self.graph.node(node_name, label=node.name, color="green", style="filled", fillcolor="lightgreen")
        else:
        ...
        if node.suffix_link and node.suffix_link != self.trie.root:
            ...
            self.graph.edge(node_name, suffix_name, style="dashed", color="blue", label="suffix")
        ...
        if node.terminal_link and node.terminal_link != self.trie.root:
            ...
            self.graph.edge(node_name, terminal_link_name, style="dashed", color="red", label="term")
