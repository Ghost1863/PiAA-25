...
class Trie:
    ...
    def _create_suffix_link_for_node(self, node):
        ...
        print(f"  Current suffix link of parent '{node.parent.name}': '{link.name if link else 'None'}'")
        ...
        if link:
            ...
            print(f"  Set suffix link for node '{node.name}' -> '{link.children[node.name].name}'")
            ...
            if node:
                ...
                while temp:
                    if temp.terminate:
                        ...
                        result.append([index - len(self.terminate_patterns[temp.terminate]) + 2,temp.terminate])
