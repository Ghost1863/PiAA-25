...
class MatrixCreator:
    ...
    def generate_euclidean_matrix(self, size: int, dimensions: int = 2, min_val: int = 0, max_val: int = 10):
        # Генерация случайных точек в n-мерном пространстве
        points = [[random.randint(min_val, max_val) for _ in range(dimensions)] for _ in range(size)]
