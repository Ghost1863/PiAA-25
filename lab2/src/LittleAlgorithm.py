...
class LittleAlgorithm:
    ...
    def collect_tree_data(self, current_cost: int, parent_node: str, step_cost: float, i_index: list[int],
                          j_index: list[int], branching_arc: tuple[int, int] = None) -> str:
    ...
    def method_Little(self, matrix: list[list], tmp_solution: dict, cur_cost: int, i_index: list[int],
                      j_index: list[int], parent_node: str = None, branching_arc: tuple[int, int] = None) -> None:
        ...
        node_id = self.collect_tree_data(current_cost, parent_node, step_cost,i_index,j_index, branching_arc)

        if current_cost >= self.min_cost:
            print("Текущая стоимость пути уже не будет выгоднее чем рекорд, конец рекурсии", self.min_cost)
            return
        ...
        self.matrix_handler.delete_row_column(i, j, new_solution, left_i_index, left_j_index)
        self.method_Little(left_matrix, new_solution, current_cost, left_i_index, left_j_index, node_id,
