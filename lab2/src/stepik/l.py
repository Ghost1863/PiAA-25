        ...
        for i in range(len(self.matrix)):
            for j in range(len(self.matrix)):
                if self.matrix[i][j] == 0:
                    ...
                    tmp = self.min_except(self.matrix[i], j) + self.min_except([row[j] for row in self.matrix], i)
...
class LittleAlgorithm:
    ...
    def method_Little(self, matrix: list[list], tmp_solution: dict, cur_cost: int, i_index: list[int],j_index: list[int]):
        ...
        self.method_Little(left_matrix, new_solution, current_cost, left_i_index, left_j_index)
