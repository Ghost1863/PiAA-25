...
class ADO_MOD_algorithm:
    ...
    def prim(self, start) -> None:
        ...
        for _ in range(n):
            u = min((i for i in range(n) if not in_mst[i]), key=lambda i: key[i], default=-1)
            if u == -1:
                break
            in_mst[u] = True
            for v in range(n):
                if not in_mst[v] and self.matrix[u][v] != -1 and self.matrix[u][v] < key[v]:
    ...
    def dfs(self, u, adj, visited, path):
        visited.add(u)
        path.append(u)
        for v in sorted((v for v in adj[u] if v not in visited), key=lambda v: self.matrix[u][v]):
